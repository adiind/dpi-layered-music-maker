    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-4 text-[20px] font-medium text-[#202124]">{pendingOption?.name ?? currentOption?.name}</p>
          <p className="mt-1 min-h-4 text-[12px] text-[#5f6368]">
            {pendingOption ? "Queued for next bar" : currentOption?.tone}
          </p>
        </div>
        <div
          className={classNames(
        ...
        {layer.options.map((option) => {
          ...
          return (
            <button
              ...
              className={classNames(
                "flex min-h-11 items-center justify-between rounded-lg border px-3 text-left text-[13px] transition",
                selected
                  ? "border-transparent bg-[#f8fafd] text-[#202124]"
                  : "border-[#e8eaed] bg-white text-[#5f6368] hover:border-[#dadce0] hover:bg-[#f8fafd]",
              )}
              style={{
                borderColor: selected || pending ? layer.accent : undefined,
                boxShadow: selected || pending ? `inset 3px 0 0 ${layer.accent}` : undefined,
              }}
            >
              <span>{option.name}</span>
              {pending ? (
                <span className="text-[11px] font-medium" style={{ color: layer.accent }}>
                  next
                </span>
              ) : null}
            </button>
