    ...
    <section className="flex h-full min-h-[520px] flex-col rounded-lg border border-[#e8eaed] bg-white p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-[12px] text-[#5f6368]">120 BPM shared grid</p>
        </div>
        ...
        {changeNotice ? (
          <div>
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] text-[#5f6368]">{changeNotice.timingLabel}</span>
            </div>
        ...
        <div className="relative h-64 w-64">
          ...
          <div className="absolute inset-0 grid place-items-center">
            <div className="text-center">
              <div className="text-[28px] font-medium text-[#202124]">{activeLayers.length}</div>
              <div className="text-[11px] uppercase tracking-[0.14em] text-[#5f6368]">active</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-2">
        {LAYER_DEFINITIONS.map((layer) => {
          ...
          return (
            <div
              key={layer.id}
              className="flex h-10 items-center justify-between rounded-lg border border-[#e8eaed] bg-[#fbfcfe] px-3 text-[13px]"
            >
              <div className="flex min-w-0 items-center gap-3">
                <span className="h-2 w-2 rounded-full" style={{ backgroundColor: muted ? "#dadce0" : layer.accent }} />
